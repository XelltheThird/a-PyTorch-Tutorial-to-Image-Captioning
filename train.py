...
def main(path="", name=""):
    ...
    encoder_lr = 0.01#1e-4  # learning rate for encoder if fine-tuning
    decoder_lr = 0.01#4e-4  # learning rate for decoder
    ...
    for epoch in range(start_epoch, epochs):
        ...
        train_accuracy = train(train_loader=train_loader,
              ...
              epoch=epoch, device=device, use_doubly_stochastic_attention=use_doubly_stochastic_attention, alpha_c=alpha_c, grad_clip=grad_clip, print_freq=print_freq)
...
def train(train_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer, epoch, device, use_doubly_stochastic_attention, alpha_c, grad_clip, print_freq):
    """
    ...
    for i, (imgs, caps, caplens) in enumerate(train_loader):
        ...
        #scores, _ = pack padded seq
        import numpy as np
        x = pack_padded_sequence(scores, decode_lengths, batch_first=True)
        
        #targets, _
        y = pack_padded_sequence(targets, decode_lengths, batch_first=True)
